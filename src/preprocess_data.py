...
from dqn_src.traffic_indicator import TrafficIndicatorFeature as feaTure
from dqn_src.buildseq import build_timeseries
...
class PreporcessDataset:
    ...
    def get_fet_cols(self):
        ...
        torch.save(self.fet_cols, self.data_dir, 'feature_columns.pt')
...
if __name__ == '__main__':
    ...
    print("[{}] Start data preprocessing ...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    base_dir = "/scratch/achen/COS561/deepqueuenet/DeepQueueNet-synthetic-data/data"
    '''
    print("[{}] Creating Preprocess Dataset class...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset = PreporcessDataset(base_dir, mode="train")
    print("[{}] Building output folders...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset.build_folder()
    print("[{}] Preprocessing data...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset.data_preprocess()
    print("[{}] Getting feature columns...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset.get_fet_cols()
    # Function below failed bc unable to find data in directories with '_merged'
    print("[{}] Sampling validation data...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset.sample_validation_data()
    print("[{}] Converting to torch...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset.sampled_valid_h5py_to_torch()
    print("[{}] Preprocessing done ...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    '''
    print("[{}] Creating dataset class...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset = PreporcessDataset(base_dir, mode='train')
    print("[{}] Calculating min and max...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    feature_columns = torch.load(os.path.join(base_dir, 'feature_columns.pt'))
    dataset.fet_cols = feature_columns
    print("Feature columns: {}".format(dataset.fet_cols))
    dataset.cal_min_max()
    print("[{}] Loading scaler...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset.load_scaler()
    print("[{}] Merging and normalizing...".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    dataset.merge_and_normalize(mode='train')
